...
export const MIN_LIVING_AREA = 10;
...
export const MIN_INDOOR_PARKING_SPACES = 1;
export const MAX_INDOOR_PARKING_SPACES = 6;
...
export const MIN_NUMBER_OF_UNITS = 3;
export const MAX_NUMBER_OF_UNITS = 25;
...
const useValidation = () => {
  ...
  return {
    ...
    title: Yup.string()
      ...
      .test(
        "len",
        "Title should not contain only digits",
        (val) => Number(val?.replace(/[0-9]/g, "").length) > 0
      ),
    ...
    property: Yup.object({
      ...
      buildingYear: Yup.number()
        .typeError("Building year must be a number")
      ...
      livingArea: Yup.number()
        .typeError("Net living area must be a number")
        .min(
          MIN_LIVING_AREA,
      ...
      balconyArea: Yup.number()
        .typeError("Balalcony area must be a number")
        .min(
          MIN_BALCONY_AREA,
      ...
      gardenArea: Yup.number()
        .typeError("Garden area must be a number")
        .min(
          MIN_GARDEN_AREA,
      ...
      numberOfIndoorParkingSpaces: Yup.number()
        .typeError("Garage space must be a number")
        .min(
          MIN_INDOOR_PARKING_SPACES,
          `Number of garage spaces should not be less than ${MIN_INDOOR_PARKING_SPACES}`
        )
        .max(
          MAX_INDOOR_PARKING_SPACES,
          `Number of garage spaces should not be more than ${MAX_INDOOR_PARKING_SPACES}`
        ),
      landArea: Yup.number()
        .typeError("Land area must be a number")
        .when("propertyType.code", {
          is: (value: string) => {
            return ["house", "multi_family_house"].includes(value);
          },
          then: Yup.number()
            .min(
              MIN_LAND_AREA,
      ...
      numberOfUnits: Yup.number()
        ...
        .when("propertyType.code", {
          is: (value: string) => {
            ...
            // console.log(
            //   '["multi_family_house"].includes(value)',
            //   ["multi_family_house"].includes(value)
            // );
            return ["multi_family_house"].includes(value);
          },
          then: Yup.number()
            .min(
              MIN_NUMBER_OF_UNITS,
              `Number of units must not be less than ${MIN_NUMBER_OF_UNITS}`
            )
            .max(
              MAX_NUMBER_OF_UNITS,
              `Number of units must not be greater than ${MAX_NUMBER_OF_UNITS}`
            )
            .required("Number of units is required"),
        }),
      annualRentIncome: Yup.number()
        .typeError("Annual rent income must be a number")
        .min(
